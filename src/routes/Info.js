...
export default function Info() {
    ...
    return (
        <PageContainer $lang={lang}>
            <h2>{ t("chapters:info") }</h2>

            <LineLang />

            <span>
                <p style={{ paddingBottom: '0', textTransform: 'uppercase', fontWeight: lang === 'ru' ? '400' : '500' }}>{ t( list_01 + "part_01") }</p>

                <ul>
                    <li>
                        <div style={{ display: 'flex', flexDirection: 'row', alignItems: 'center'}}>
                            <div><p style={{ marginRight: '10px'}}>{ t(list_01 + "part_02") }</p></div>
                            <Link to={"/contacts"}><ButtonController variant={"info"} /></Link>
                        </div>
                    </li>
                    <li><p>{ t(list_01 + "part_03") }</p></li>
                    <li><p>{ t(list_01 + "part_04") }</p></li>
                    <li><p>{ t(list_01 + "part_05") }</p></li>
                    <li><p>{ t(list_01 + "part_06") }</p></li>
                </ul>

                <p style={{ fontWeight: '600' }}>{ t( text + "info_part_01") }</p>

                <InlineParagraphWithButton>
                    <div className={"inlineParagraphWithButton"}>
                        <p>{ t(list_02 + "part_01") }</p>
                         <p>{ repeatSign }</p>
                    </div>
                ...
                <ul style={{ paddingTop: '0'}}>
                    <li><p>{ t(list_02 + "part_02") }</p></li>
                    <li><p>{ t(list_02 + "part_03") }</p></li>
                    <li><p>{ t(list_02 + "part_04") }</p></li>
                </ul>
            </span>
